...
In editing, while previewing with `npm run dev`, hovering a navigation bar item that has a real destination link shows that page's embedded OGP image like a tooltip.
When the pointer leaves the menu item, the preview image disappears automatically.
This behavior is preview-only and is not included in the normal static site generated by `npm run build`.
...
#### Fonts for OGP images

The standard `.github/workflows/build.yml` generated by `atr init` installs `fonts-noto` for OGP image rendering.
This makes it easy to use the `Noto Sans` family referenced by the default scaffold `fontList` on GitHub Actions as well.
...
If you already know the package name you need, or want to reduce build time, you can replace `OGP_FONT_PACKAGES` with a smaller package set.
For example, you can install `fonts-noto-cjk` or `fonts-ipaexfont` instead of `fonts-noto`.

If you want to use a custom font in OGP images, update `build.yml` so it installs the corresponding font package.
You also need to adjust `variables.fontList`, because it is what the SVG templates render into `font-family`.

This font-loading customization is only needed for OGP PNG generation.
Normal HTML/CSS rendering is resolved by the browser font environment, so the same setup is not required there.
