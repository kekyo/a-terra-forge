...
CLIをアップデートした後、編集スペース側の `.assets/` や `.templates/` に含まれる標準ファイルも新しいものへ揃えたい場合は、 `atr update` コマンドを使用します:

```bash
$ atr update
```

このコマンドは、標準の雛形として管理されている `assetsDir` と `templatesDir` 配下のファイルだけを上書きします。`docs/` などの文書ファイルは更新対象になりません。
...
|`templateNames`|No| `templatesDir` 配下で優先順に探索するテンプレートディレクトリ名の一覧です。デフォルトは `["default"]` です。例えば `["great", "default"]` とすると、まず `.templates/great/` を参照し、存在しないファイルは `.templates/default/` にフォールバックします。 |
...
`great` にあるファイルが優先され、存在しないものだけが `default` にフォールバックします。
