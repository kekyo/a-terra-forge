...
const resolveTimelinePrerenderCount = (
  variables: FunCityVariables
): number | undefined => {
  const raw = variables.get('timelinePrerenderCount');
...
export const generateTimelineDocument = async (
  ...
  const timelinePrerenderCount = resolveTimelinePrerenderCount(configVariables);
  ...
  const contentVariables = {
    ...
    getTimelineEntry,
    ...(timelinePrerenderCount !== undefined ? { timelinePrerenderCount } : {}),
  };
